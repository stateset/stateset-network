...
data class Invoice(val invoiceNumber: String,
    ...
    override val participants: List<Party> get() = listOf(party, counterparty, factor)
    fun withNewParty(newParty: Party) = copy(party = newParty)
    fun withNewCounterparty(newCounterparty: Party) = copy(counterparty = newCounterparty)
    fun withNewFactor(newFactor: Party) = copy(factor = newFactor)
    ...
    override fun toString(): String {
        ...
        val factorString = (factor as? Party)?.name?.organisation ?: factor.owningKey.toBase58String()
        return "Invoice ($linearId): $counterpartyString has an invoice with $partyString for $total which is being factored by $factorString and the current paid status is $paid."
    }
    ...
    override fun generateMappedObject(schema: MappedSchema): PersistentState {
        return when (schema) {
            is InvoiceSchemaV1 -> InvoiceSchemaV1.PersistentInvoice(
                    ...
                    factor = this.factor.name.toString(),
...
class InvoiceContract : Contract {
    ...
    interface Commands : CommandData {
        ...
        class TransferInvoice : TypeOnlyCommandData(), Commands
    ...
    override fun verify(tx: LedgerTransaction) {
        ...
        when(invoiceCommand.value) {
            ...
            is Commands.TransferInvoice -> requireThat {
                "one input should be produced" using (invoiceInputs.size == 1)
                "one output should be produced" using (invoiceOutputs.size == 1)
